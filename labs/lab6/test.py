...
filter_ken = join_map.filter(lambda x: re.match(r'(ken.lay|kenneth.lay|lay.ken).*', x[1]))
print 'ken', sorted(filter_ken.collect(), key=lambda x: x[2], reverse=True)[:10]

filter_jeff = join_map.filter(lambda x: re.match(r'(jeff.skilling|jefferey.skilling|skilling.jeff).*', x[1]))
print 'jeff', sorted(filter_jeff.collect(), key=lambda x: x[2], reverse=True)[:10]

filter_andrew = join_map.filter(lambda x: re.match(r'(andrew.fastow|andy.fastow|fastow.andy).*', x[1]))
